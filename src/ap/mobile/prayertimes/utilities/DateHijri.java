...
public class DateHijri {
    ...
    public static String convert(Calendar calendar) {
    	...
    	String[] wdNames = { "Minggu", "Senin", "Selasa", "Rabu", "Kamis",
                "Jumat", "Sabtu" };
        String[] iMonthNames = { "Muharram", "Safar", "Rabiul Awwal",
                "Rabiul Akhir", "Jumadil Awal", "Jumadil Akhir", "Rajab",
                "Syaban", "Ramadhan", "Syawal", "Dzulkaidah", "Dzulhijjah" };
        ...
        double[] iDate = new double[8];

        iDate[0] = day; // calculated day (CE)
        iDate[1] = month - 1; // calculated month (CE)
        iDate[2] = year; // calculated year (CE)
        iDate[3] = jd - 1; // julian day number
        iDate[4] = wd - 1; // weekday number
        iDate[5] = id; // islamic date
        iDate[6] = im - 1; // islamic month
        iDate[7] = iy; // islamic year
        
        return wdNames[(int) iDate[4]] + ", " + (int) iDate[5]
                + " " + iMonthNames[(int) iDate[6]] + " " + (int) iDate[7] + " H";
    }
